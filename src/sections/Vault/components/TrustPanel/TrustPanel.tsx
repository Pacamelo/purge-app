...
export const TrustPanel = memo(function TrustPanel({
  ...
  return (
    <div className="h-full flex flex-col">
      {/* ─────────────────────────────────────────────────────────────────
          PERSISTENT SECURITY WARNING - Always visible (redaction bar style)
          ───────────────────────────────────────────────────────────────── */}
      <div className="relative p-3 bg-[var(--forge-warning)]/10 border-b-2 border-[var(--forge-warning)]">
        {/* Redaction bar accent */}
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-[var(--forge-warning)]" />
        <p className="text-xs font-mono text-[var(--forge-warning)] leading-relaxed pl-2">
      ...
      {/* ─────────────────────────────────────────────────────────────────
          MAIN STATUS CARD - The one thing that matters (redaction bar style)
          ───────────────────────────────────────────────────────────────── */}
      <div className="p-4">
        <div
          className={`
            relative p-4 transition-all overflow-hidden
            ${showSuccess ? 'bg-forge-success/5' : ''}
            ${showReady ? 'bg-white/5' : ''}
            ${showWarning ? 'bg-forge-error/5' : ''}
            ${!showSuccess && !showReady && !showWarning ? 'bg-forge-bg-tertiary' : ''}
          `}
        >
          {/* Redaction bar borders */}
          <div className={`absolute left-0 top-0 bottom-0 w-1 ${showSuccess ? 'bg-forge-success' : showReady ? 'bg-white' : showWarning ? 'bg-forge-error' : 'bg-forge-border'}`} />
          <div className={`absolute right-0 top-0 bottom-0 w-1 ${showSuccess ? 'bg-forge-success' : showReady ? 'bg-white' : showWarning ? 'bg-forge-error' : 'bg-forge-border'}`} />
          <div className={`absolute top-0 left-0 right-0 h-1 ${showSuccess ? 'bg-forge-success' : showReady ? 'bg-white' : showWarning ? 'bg-forge-error' : 'bg-forge-border'}`} />
          <div className={`absolute bottom-0 left-0 right-0 h-1 ${showSuccess ? 'bg-forge-success' : showReady ? 'bg-white' : showWarning ? 'bg-forge-error' : 'bg-forge-border'}`} />
          {/* Status Header */}
